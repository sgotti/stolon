...
func TestUpdateCluster(t *testing.T) {
	tests := []struct {
		...
		// #13 One master and one standby. keeper2 dead for more then DeadKeeperRemovalInterval: keeper2 removed (and also its associated db)
		{
			cd: &cluster.ClusterData{
				Cluster: &cluster.Cluster{
					UID:        "cluster1",
				...
				DBs: cluster.DBs{
					...
					"db2": &cluster.DB{
						...
						Status: cluster.DBStatus{
							// TODO(sgotti) keeper removed also if db is healthy, but this shouldn't happen in the reality since db healthy is set to false when keeper isn't healthy
							Healthy:           true,
							CurrentGeneration: 1,
						},
					},
