...
func (p *PostgresKeeper) generateHBA(cd *cluster.ClusterData, db *cluster.DB) []string {
	...
	case cluster.SUReplAccessAll:
		// all the keepers will accept connectios from every host
		computedHBA = append(
			computedHBA,
	...
	case cluster.SUReplAccessStrict:
		// only the master keeper (primary instance or standby of a remote primary when in standby cluster mode) will accept connectios only from the other standby keepers IPs
		if IsMaster(db) {
